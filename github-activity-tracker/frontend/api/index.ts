...
// ✅ FIX: warn if token missing
if (!GITHUB_TOKEN) {
  console.warn('⚠️ GITHUB_TOKEN is not set. GitHub API calls may be rate-limited or fail.');
}

const execPromise = promisify(exec);

// Paths (unchanged)
const CONFIG_PATH = path.resolve(__dirname, '../../backend/config.properties');
const PYTHON_SCRIPT_PATH = path.resolve(__dirname, '../../backend/github_activity.py');
...
async function checkRepoExists(repoName: string): Promise<boolean> {
  ...
  try {
    const response = await fetch(url, {
      headers: {
        // ✅ FIX: only include token if exists
...
async function executePythonScript(): Promise<CommandResult> {
  ...
  try {
    ...
    return await runCommand(`python ${PYTHON_SCRIPT_PATH}`);
  } catch (error) {
    console.error("❌ Python script not found or failed:", error);
    return {
      stdout: '',
      stderr: 'Python script not found or execution failed'
    };
  }
}
...
app.delete('/api/removeRepo/:repoName', async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    await fs.writeFile(CONFIG_PATH, filteredLines.join('\n'));
